...
import itertools
# This is basically the top level pmpnn file from the pmpnn repo but with some changes to adapt it to more sane usage

# A number of functions/classes are adopted from: https://github.com/jingraham/neurips19-graph-protein-design

def get_weird_pmpnn_stuff(chain_idx):
    L = len(chain_idx)
    residue_idx = -100 * torch.ones(L,  dtype=torch.long)  # residue idx with jumps across chains
    chain_encoding = torch.zeros(L,  dtype=torch.long)
    unique_values, counts = torch.unique(chain_idx, return_counts=True)
    count_cumsum = torch.cat([torch.zeros(1), torch.cumsum(counts, 0)]).long()
    for j, count in enumerate(counts):
        residue_idx[count_cumsum[j]:count_cumsum[j + 1]] = 100 * (j - 1) + torch.arange(count)
        chain_encoding[count_cumsum[j]:count_cumsum[j + 1]] = j
    return residue_idx, chain_encoding
...
class ProteinMPNN(nn.Module):
    def __init__(self, args, node_features=128, edge_features=128,
                 hidden_dim=128, num_encoder_layers=3, num_decoder_layers=3,
                 vocab=21, k_neighbors=48, augment_eps=0.1, dropout=0.1, ca_only=False):
        ...
        if self.args.taxon_condition:
            self.taxon_emb = nn.Embedding(self.args.num_taxon_ids, hidden_dim)
    ...
    #def forward_train(self, X, S, taxon_id, mask, chain_M, residue_idx, chain_encoding_all):
    def forward_train(self, X, S, taxon_id, mask, chain_M, residue_idx, chain_encoding_all, return_hidden=False):
        ...
        decoding_order = torch.argsort((chain_M + 0.0001) * (torch.abs(torch.randn(chain_M.shape,
                                                                                   device=device))))  # [numbers will
        # be smaller for places where chain_M = 0.0 and higher for places where chain_M = 1.0]
        ...
        # ========== ??????==========
        if return_hidden:
            return log_probs, h_V_encoder, h_V  # log_probs, ??? ????        else:
            return log_probs
            
            
    def forward_inference(self, X, S, taxon_id, mask, chain_M, residue_idx, chain_encoding_all, randn,
        ...
        if not use_input_decoding_order:
            decoding_order = torch.argsort((chain_M + 0.0001) * (torch.abs(
                randn)))  # [numbers will be smaller for places where chain_M = 0.0 and higher for places where
            # chain_M = 1.0]
        mask_size = E_idx.shape[1]
        permutation_matrix_reverse = torch.nn.functional.one_hot(decoding_order, num_classes=mask_size).float()
        order_mask_backward = torch.einsum('ij, biq, bjp->bqp',
    ...
    def sample(self, X, randn, S_true, taxon_id, chain_mask, chain_encoding_all, residue_idx, mask, temperature,
               omit_AAs_np, bias_AAs_np, chain_M_pos, bias_by_res):
        device = X.device
        # Prepare node and edge embeddings
        E, E_idx = self.features(X, mask, residue_idx, chain_encoding_all)
        ...
        decoding_order = torch.argsort((chain_mask + 0.0001) * (torch.abs(
            randn)))  # [numbers will be smaller for places where chain_M = 0.0 and higher for places where chain_M =
        # 1.0]
        mask_size = E_idx.shape[1]
        permutation_matrix_reverse = torch.nn.functional.one_hot(decoding_order, num_classes=mask_size).float()
        order_mask_backward = torch.einsum('ij, biq, bjp->bqp',
        ...
        all_probs = torch.zeros((N_batch, N_nodes, self.vocab), device=device, dtype=torch.float32)
        ...
        for t_ in range(N_nodes):
            ...
            if (mask_gathered == 0).all():  # for padded or missing regions only
                ...
                probs = F.softmax(logits - constant[None, :] * 1e8 + constant_bias[None,
                                                                     :] / temperature + bias_by_res_gathered /
                                  temperature,
                                  dim=-1)
                S_t = torch.multinomial(probs, 1)
                all_probs.scatter_(1, t[:, None, None].repeat(1, 1, self.vocab),
    ...
    def tied_sample(self, X, randn, S_true, chain_mask, chain_encoding_all, residue_idx, mask=None, temperature=1.0,
        ...
        N_batch, N_nodes = X.size(0), X.size(1)
        log_probs = torch.zeros((N_batch, N_nodes, self.vocab), device=device)
        all_probs = torch.zeros((N_batch, N_nodes, self.vocab), device=device, dtype=torch.float32)
        h_S = torch.zeros_like(h_V, device=device)
        S = torch.zeros((N_batch, N_nodes), dtype=torch.int64, device=device)
        h_V_stack = [h_V] + [torch.zeros_like(h_V, device=device) for _ in range(len(self.decoder_layers))]
    ...
    def conditional_probs(self, X, S, mask, chain_M, residue_idx, chain_encoding_all, randn, backbone_only=False):
        ...
        log_conditional_probs = torch.zeros([X.shape[0], chain_M.shape[1], self.vocab], device=device).float()
